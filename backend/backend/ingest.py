...
from security import PIIDetector, AdaptiveRateLimiter
...
class SyslogListener:
    ...
    def __init__(self, port: int = 5140, callback: Optional[Callable] = None):
        ...
        self.rate_limiter = AdaptiveRateLimiter(base_limit=500)
        ...
        self.pii_detections = 0  # Track PII detection count
    ...
    async def start(self):
        ...
        while self.running:
            try:
                ...
                # Rate limiting
                allowed, reason = self.rate_limiter.allow(source_ip)
                if not allowed:
                    print(f"[SYSLOG] {source_ip}: {reason}")
                    continue
    ...
    def _parse_syslog(self, message: str, source_ip: str) -> Optional[Event]:
        ...
        try:
            ...
            pii_found = self.pii_detector.scan(message)
            if pii_found:
                self.pii_detections += 1
                message, was_redacted, pii_types = self.pii_detector.redact(message)
                parsed['pii_redacted'] = was_redacted
                parsed['pii_types'] = pii_types
                parsed['message'] = message  # Update with redacted message
                print(f"[SECURITY] PII detected in syslog from {source_ip}: {', '.join(pii_types)}")
...
class LogFileHandler(FileSystemEventHandler):
    ...
    def __init__(self, filepath: Path, callback: Callable, pii_detector: PIIDetector):
    ...
    def _parse_log_line(self, line: str) -> Optional[Event]:
        ...
        try:
            ...
            # PII Detection and Redaction
            pii_found = self.pii_detector.scan(line)
            if pii_found:
                line, was_redacted, pii_types = self.pii_detector.redact(line)
                parsed['pii_redacted'] = was_redacted
                parsed['pii_types'] = pii_types
                parsed['message'] = line
                print(f"[SECURITY] PII detected in file {self.filepath.name}: {', '.join(pii_types)}")
...
class FileTailWatcher:
    ...
    def add_file(self, filepath: str):
        ...
        handler = LogFileHandler(path, self._handle_event, self.pii_detector)
    ...
    async def _handle_event(self, event: Event):
        ...
        if event.parsed.get('pii_redacted'):
            self.pii_detections += 1
...
class WebhookReceiver:
    ...
    def __init__(self, callback: Optional[Callable] = None):
        ...
        self.events_received = 0
        self.last_event: Optional[datetime] = None
        self.rate_limiter = AdaptiveRateLimiter(base_limit=100)
        self.pii_detector = PIIDetector(
            redact_email=False,
            redact_phone=False,
            redact_internal_ips=False
    ...
    async def receive_event(self, data: dict, source_id: str = "unknown") -> Event:
        ...
        # Rate limiting
        allowed, reason = self.rate_limiter.allow(source_id)
        if not allowed:
        ...
        # PII Detection and Redaction
        pii_found = self.pii_detector.scan(raw_str)
        if pii_found:
            self.pii_detections += 1
            # Redact PII from data dictionary fields
            for key, value in data.items():
                if isinstance(value, str):
                    _, was_redacted, pii_types = self.pii_detector.redact(value)
                    if was_redacted:
                        data[key], _, _ = self.pii_detector.redact(value)
            print(f"[SECURITY] PII detected in webhook from {source_id}")
        ...
        # Add PII metadata if detected
        if pii_found:
            event.parsed['pii_redacted'] = True
            event.parsed['pii_types'] = list(pii_found.keys())
